...
# Render Web Service persistent disk is mounted at /var/data by render.yaml.
# Keep DATA_DIR configurable so the same code can still run locally/Railway.
DATA_DIR = Path(
    os.environ.get(
        "DATA_DIR",
        "/var/data" if RENDER_HOSTNAME else os.environ.get(
...
async def startup():
    ...
    # Raw VLESS-TCP requires a separately exposed public TCP listener.
    # Render Web Service does not expose an arbitrary public TCP port, so keep
    # this relay disabled by default on Render. WS/XHTTP continue on $PORT.
    tcp_relay_enabled = os.environ.get(
        "ENABLE_TCP_RELAY",
        "0" if os.environ.get("RENDER_EXTERNAL_HOSTNAME") else "1",
    ...
    if tcp_relay_enabled:
        ...
        logger.info("VLESS-TCP raw listener disabled; using Render HTTP/WebSocket $PORT")
...
async def shutdown():
    ...
    if os.environ.get("ENABLE_TCP_RELAY", "").strip().lower() in ("1", "true", "yes", "on"):
        try:
            import tcp_relay
...
async def railway_network_info(_=Depends(require_auth)):
    """Return deployment networking hints without exposing secrets.

    The legacy route name is retained for frontend compatibility.
    """
    ...
    return {
        "provider": "render" if is_render else ("railway" if is_railway else "generic"),
        "is_render": is_render,
        "is_railway": is_railway,
        "public_domain": os.environ.get("RENDER_EXTERNAL_HOSTNAME", "") or os.environ.get("RAILWAY_PUBLIC_DOMAIN", ""),
        "tcp_proxy_domain": "",
        "tcp_proxy_port": 0,
        "tcp_application_port": 0,
        "app_port": safe_int(os.environ.get("PORT", "0"), minimum=0, maximum=65535),
        "websocket_public_port": 443 if is_render else 0,
        "raw_tcp_supported": not is_render,
    }
